...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // radial portal glow
      const rg = ctx.createRadialGradient(w / 2, h / 2, 0, w / 2, h / 2, Math.min(w, h) / 1.3);
  ...
  function friendlyAuthMessage(resStatus: number | null, data: any) {
    // server might return structured errors in data.message or data.error
    const serverMsg = (data?.message || data?.error || "").toString().toLowerCase();
    ...
    if (serverMsg.includes("not_found") || serverMsg.includes("notfound") || serverMsg.includes("not-found")) return "البريد الإلكتروني أو كلمة السر خاطئان";

    // otherwise show server msg if meaningful
    if (serverMsg) return data?.message ?? data?.error ?? "خطأ أثناء تسجيل الدخول";
  ...
  return (
    <main dir="rtl" style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <img src="/assets/tojar.png" alt="tojar" style={{
                width: 56, height: 56, borderRadius: 12, objectFit: "cover",
                boxShadow: "0 12px 34px rgba(11,92,255,0.18)", transform: "translateZ(28px)"
              }} />
              <div>
                <div style={{ fontSize: 18, fontWeight: 900, color: "#e6f7ff", transform: "translateZ(24px)" }}>شركة تجار العالمية</div>
                <div style={{ fontSize: 13, color: "#bfe9ff", marginTop: 2 }}>مرحباً بك في شركة تجار العالمية</div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12 }}>
              ...
              <label style={{ display: "grid", gap: 6 }}>
                ...
                <div style={{
                  ...
                  <input
                    ...
                    autoComplete="email"
                    style={{
                      width: "100%", padding: "14px 16px", background: "transparent", color: "#e6f7ff",
              ...
              <label style={{ display: "grid", gap: 6 }}>
                ...
                <div style={{
                  ...
                  <input
                    ...
                    style={{
                      width: "100%", padding: "14px 54px 14px 16px", background: "transparent", color: "#e6f7ff",
                      border: "none", outline: "none", fontSize: 15
                    }}
              ...
              {msg && (
                <div role="alert" style={{
                  ...
                  <img src="/assets/notification.png" alt="notif" style={{ width: 28, height: 28, objectFit: "contain" }} onError={(e)=>{ (e.currentTarget as HTMLImageElement).style.display='none'; }} />
                  <div>{msg}</div>
                </div>
              )}
            </form>
      ...
      {/* corner glows */}
      <div style={{
        position: "fixed", right: -80, top: -80, width: 260, height: 260, zIndex: 1,
        borderRadius: "50%", background: "radial-gradient(circle, rgba(6,182,212,0.28), rgba(6,182,212,0) 70%)", pointerEvents: "none"
      }} />
      <div style={{
        position: "fixed", left: -100, bottom: -100, width: 340, height: 340, zIndex: 1,
        borderRadius: "50%", background: "radial-gradient(circle, rgba(255,209,102,0.22), rgba(255,209,102,0) 70%)", pointerEvents: "none"
      }} />
    </main>
  );
}