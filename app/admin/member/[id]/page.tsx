...
export default function MemberDetailPage() {
  ...
  async function fetchDetail() {
    ...
    try {
      ...
      const l1 = j.referrals?.level1 || [];
      const l2 = j.referrals?.level2 || [];
      setLevel1(l1);
      setLevel2(l2);

      // ابني حالات التحديد من الحقل persisted invited_selected
      const s1: Record<string, boolean> = {};
      l1.forEach((x: any) => { s1[x.id] = !!x.invited_selected; });
      setSelectedL1(s1);

      const s2: Record<string, boolean> = {};
      l2.forEach((x: any) => { s2[x.id] = !!x.invited_selected; });
      setSelectedL2(s2);
    } catch (err) {
  ...
  async function doAction(action: 'approve' | 'reject' | 'delete') {
    if (!confirm(`تأكيد ${action}`)) return;
    try {
      const resp = await fetch(`/api/admin/members/${id}`, {
        method: 'POST',
  ...
  async function toggleSelected(level: 1 | 2, memberId: string) {
  ...
  async function batchActionOnSelected(level: 1 | 2, action: 'approve' | 'reject' | 'delete') {
    ...
    if (!confirm(`تأكيد ${action} على ${selectedIds.length} عضوا؟`)) return;

    setBatchProcessing(true);
    try {
      // نستخدم batch-select endpoint لحفظ invited_selected إذا لزم
      // ثم نطبّق action على العناصر (يمكن تعديل لطلب واحد كبير حسب الحاجة)
      // نفّذ تحديث الحالة لكل عنصر متسلسلًا (من السهل، مستقبلاً نجمع الطلبات)
      for (const memberId of selectedIds) {
        // تحديث الحالة (approve/reject/delete)
  ...
  function toggleSelectAll(level: 1 | 2, value: boolean) {
    if (level === 1) {
      ...
      level1.forEach(x => { s[x.id] = value; });
      setSelectedL1(s);
    } else {
      const s: Record<string, boolean> = {};
      level2.forEach(x => { s[x.id] = value; });
      setSelectedL2(s);
    }
  }
